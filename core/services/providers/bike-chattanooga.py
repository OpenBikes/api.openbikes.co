...
class Provider(BaseProvider):
    ...
    def get_raw_updates(self, city_api_name=None) -> list:
        r = requests.get('http://www.bikechattanooga.com/stations/json')
        r.raise_for_status()
    ...
    def parse_raw_update(self, raw_update: dict) -> station_updates.StationUpdate:
        return station_updates.StationUpdate(
            ...
            is_available=raw_update['status'] == 'IN_SERVICE',
            at=dt.datetime.strptime(raw_update['lastCommunicationTime'], '%Y-%m-%d %H:%M:%S')
        )
